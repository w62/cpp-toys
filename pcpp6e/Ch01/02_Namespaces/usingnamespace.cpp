...
import std;
namespace mycode {
void foo() { std::println("foo() called in the mycode namespace"); }
...
namespace FTP {
void ftp_init() { std::println("ftp_init from MyLibraries::Networking::FTP"); }
}  // namespace FTP
}  // namespace Networking
}  // namespace MyLibraries
...
int main() {
  ...
  ./usingnamespace.cpp:52:35: warning: narrowing conversion of
  'std::to_underlying<main()::PieceType>(piece)' from
  'std::underlying_type_t<main()::PieceType>' {aka 'long unsigned int'} to 'int'
  [-Wnarrowing] 52 | int underlyingValue {to_underlying(piece)}; |
  ~~~~~~~~~~~~~^~~~~~~
