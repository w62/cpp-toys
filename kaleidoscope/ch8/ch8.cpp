...
static int gettok() {
  ...
  /*
   *
   * Note that this isn’t doing sufficient error checking: it will incorrectly
   * read “1.23.45.67” and handle it as if you typed in “1.23”. Feel free to
   * extend it!
   */
...
class ExprAST {
 public:
  ...
  virtual Value *codegen() = 0;  // Value is actually llvm::Value
};
...
static std::unique_ptr<ExprAST> ParsePrimary() {
  switch (CurTok) {
    default:
      return LogError("unknown toke when expecting an expression");
    case tok_identifier:
...
/// the function. This is used fro mutable variable etc.
static AllocaInst *CreateEntryBlockAlloca(Function *TheFunction,
        StringRef VarName) {
...
Value *BinaryExprAST::codegen() {
    ...
    if (Op == '=') {
            // This asume we're building without RTTI because LLVM builds that 
  ...
  // If it wasn't a buildin binary operator, it must be a user defined one. Emit
  // a call to it.
...
Value *ForExprAST::codegen() {
  ...
  // shadows an existing variable, we have to restore it, so save it for now.
  ...
  // Create the "after loop" block and insert.
  BasicBlock *AfterBB =
...
static void InitializeModuleAndManagers() {
  ...
  // Simplify the control flow graph (deleteing unreachable blocks, etc).
